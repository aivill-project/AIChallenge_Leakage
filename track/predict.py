...
if __name__ == '__main__':
    ...
    public_valid = pd.read_csv(r'C:\workspace\pythonProject\leakage\public_test_with_truth.csv')
    public_valid.set_index('id', inplace=True)
    public_valid = public_valid.loc[sorter].reset_index()
    
    public_f1 = f1_score(public_valid['leaktype'], result['leaktype'], average='macro')
    print('')
    print(f'### Public F1: {public_f1}')
